...
class CarsIndex extends Component
{
    ...
    public function updateSearchPrice($val)
    {
        ...
        $this->Dprice = $vehicles[0] ?? '';
        $this->mprice = $vehicles[1] ?? '';
    }
    public function render()
{
  ...
  // cars seates
    ...
    // $hasPriceFilter = $this->minPrice || $this->maxPrice;
    // $vehicles->when($hasPriceFilter, function ($query) {
    //     if ($this->minPrice !== null) {
    //         $query->where('dprice', '>=', $this->minPrice);
    //     }
    //     if ($this->maxPrice !== null) {
    //         $query->where('mprice', '<=', $this->maxPrice);
    //     }
    // });
