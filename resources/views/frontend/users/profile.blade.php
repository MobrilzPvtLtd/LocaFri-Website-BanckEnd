    ...
    <div class="profile-container">
        <div class="profile-header">
            ...
            @if ($$module_name_singular->url_website)
                <a href="{{ $$module_name_singular->url_website }}" target="_blank" class="profile-website">{{ $$module_name_singular->url_website }}</a>
            @endif
        </div>
        ...
        <div class="profile-info">
            <h2>Profile</h2>
            <div class="info-item">
                <span class="info-label">{{ label_case($field_name = 'first_name') }}:</span>
                <span class="info-value">{{ $$module_name_singular->$field_name }}</span>
            </div>
            <div class="info-item">
                <span class="info-label">{{ label_case($field_name = 'last_name') }}:</span>
                <span class="info-value">{{ $$module_name_singular->$field_name }}</span>
            </div>
            @auth
                @if (auth()->user()->id == $$module_name_singular->id)
                    <div class="info-item">
                        <span class="info-label">{{ label_case($field_name = 'email') }}:</span>
                        <span class="info-value">{{ $$module_name_singular->$field_name }}</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">{{ label_case($field_name = 'mobile') }}:</span>
                        <span class="info-value">{{ $$module_name_singular->$field_name }}</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">{{ label_case($field_name = 'date_of_birth') }}:</span>
                        <span class="info-value">{{ optional($$module_name_singular->$field_name)->toFormattedDateString() }}</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">{{ label_case($field_name = 'gender') }}:</span>
                        <span class="info-value">{{ $$module_name_singular->$field_name }}</span>
                    </div>
                @endif
            @endauth
            <div class="info-item bio">
                <span class="info-label">{{ label_case($field_name = 'bio') }}:</span>
                <p class="info-value">{{ $$module_name_singular->$field_name }}</p>
            </div>
        </div>
    </div>
@endsection
